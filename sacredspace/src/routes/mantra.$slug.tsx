...
export const Route = createFileRoute("/mantra/$slug")({
  ...
  loader: async ({ params }) => {
    const data = await getContentBySlugWithDeity({ data: params.slug });
  ...
  head: ({ loaderData }) => {
    ...
    const pageUrl = `${SITE_URL}/mantra/${c.slug}`;
    ...
    const schema = {
      ...
      inLanguage: "sa",
      keywords: [c.type, d?.name, "mantra", "stotra", "Sanskrit", "Hindu"].filter(Boolean).join(", "),
    };
